<%-- 
    Document   : formularioReserva
    Created on : 12-jun-2019, 18:16:08
    Author     : oficina taxi
--%>

<%@page import="com.stevenmajek.POJO.Taxista"%>
<%@page import="com.stevenmajek.DAO.GestionDao"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <jsp:include page="bootStrap.html"></jsp:include>
            <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
            <title>Formulario</title>
        </head>
        <body class="fondo">
            <!--nav-->
            <nav class="navbar navbar-expand-lg navbar-dark fixed-top fondo">
                <img style="width: 60px; height: 60px" src="./Vistas/img/badge.png" class="mr-5"> 
                <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
                    <li class="nav-item">
                        <a class="nav-link bg-warning texto" style="color: black;" href="Controlador?accion=formulario">Editar reserva</a>                          
                    </li>
                    <li class="nav-item texto">
                        <a class="nav-link" href="Controlador?accion=formulario">Añadir reseva</a>                       
                    </li>
                    <li class="nav-item texto">
                        <a class="nav-link" href="Controlador?accion=reservas">Reservas</a>
                    </li>
                    <li class="nav-item texto">
                        <a class="nav-link" href="Controlador?accion=listaTaxis">Reservas Hechas</a>
                    </li>
                    <li class="nav-item texto">
                        <a class="nav-link texto" href="#" data-toggle="modal" data-target="#mimodal">Búsqueda</a>
                        <div class="modal fade" id="mimodal">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header fondo">
                                        <form action="Controlador">
                                            <h4 class="modal-title"><div id="acordion1" class=" my-2 ml-2">
                                                    <a class="mr-3 text-center enlace texto" style="color: #fefefe" title="Busquedad Roig" href="#demo3" data-toggle="collapse" >Roig</a>
                                                    <div id="demo3" class="collapse row">
                                                        <div class="col">
                                                            <input type="search" class="form-control mt-2" placeholder="V124356" name="busquedadRoig"> 
                                                        </div>
                                                        <div class="col">
                                                            <button type="submit" class="mt-2 btn btn-info btn-sm" name="accion" value="buscarRoig">Buscar</button>
                                                        </div>                                                                  
                                                    </div>
                                                </div>
                                            </h4>
                                        </form>
                                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                                    </div>
                                    <form class="" action="Controlador">
                                        <div class="modal-body">
                                            <div class=" row form-group justify-content-around" style="color: black;">

                                                <div class="my-2">
                                                    <label class="mr-3 text-center" for="fecha1">Desde</label>
                                                    <input type="date" class="form-control" name="fecha1">
                                                </div>

                                                <div class="my-2">
                                                    <label class="mr-3 text-center " for="fecha">Hasta</label>
                                                    <input type="date" class="form-control" name="fecha2">
                                                </div>

                                                <div class="my-2">
                                                    <label class="mr-3 text-center" for="busquedadOrigen">Origen</label>
                                                    <input type="search" class="form-control" placeholder="Blau" name="busquedadOrigen">
                                                </div>

                                                <div class="my-2">
                                                    <label class="mr-3 text-center" for="busquedadNumeroTaxi">Numero Taxi</label>
                                                    <input type="search" class="form-control" placeholder="5" name="busquedadNumeroTaxi">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <div class="col-4">
                                                <button type="submit" class="btn btn-outline-info" value="buscarHoy" name="accion">Hoy</button>
                                            </div>
                                            <div class="col-5">                                                                
                                                <button type="submit" class="btn btn-info" name="accion" value="buscarFecha">Buscar</button>
                                            </div>
                                        </div>                                     
                                    </form>
                                </div>
                            </div>
                        </div>
                    </li>
                </ul>
                <form class="form-inline" action="Controlador">
                    <input class="form-control mr-sm-2" type="search" placeholder="Search" name="inputBuscar">
                    <button class="btn bg-warning" title="buscar" value="buscarOrigen" type="search" name="accion"><img style="width: 25px; height: 25px" src="./Vistas/img/search.png"></button>
                </form>

            </nav>
            <!--end nav-->
            <div class="mb-1 mt-2 invisible">
                <span>lol</span>
            </div>
            <div class="my-5 container">
                <div class="row align-items-center">
                    <div class="col align-middle">
                        <img src="./Vistas/img/Taxi1.jpg" alt="imagen Taxi" class="rounded-circle" style="height: 600px; width: 600px">
                    </div>
                    <div class="col-4">
                    <%
                        int id = Integer.parseInt((String) request.getAttribute("id"));
                        int num = Integer.parseInt((String) request.getAttribute("num"));
                        GestionDao dao = new GestionDao();
                        Taxista t = dao.selectOneViaje(id,num);
                    %>
                    <form class="col-xs-12" action="Controlador">
                        <h2 class="text-center">Editar Reserva  <%=t.getIdReserva()%></h2>
                        <div class="form-group">
                            <input type="hidden" value="<%=t.getIdReserva()%>" class="form-control" name="idViaje">
                            <input type="hidden" value="<%=num%>" class="form-control" name="num">
                        </div>
                        <div class="form-group">
                            <label for="hora">Hora</label>
                            <input type="time" value="<%=t.getHora()%>" class="form-control" name="hora">  
                        </div>
                        <div class="form-group">
                            <label for="fecha">Fecha</label>
                            <input type="date" value="<%=t.getFecha()%>" class="form-control" name="fecha">  
                        </div>
                        <div class="form-group">
                            <label for="origen">Origen</label>
                            <input type="text" value="<%=t.getOrigen()%>" class="form-control" name="origen">
                        </div>
                        <div class="form-group">
                            <label for="habitacion">Habitación</label>
                            <input type="text" value="<%=t.getHabitacion()%>" class="form-control" name="habitacion">  
                        </div>
                        <div class="form-group">
                            <label for="destino">Destino</label>
                            <input type="text" value="<%=t.getDestino()%>" class="form-control" name="destino">  
                        </div>
                        <div class="form-group">
                            <label for="pax">Pax</label>
                            <input type="number" value="<%=t.getPax()%>" class="form-control" name="pax">
                        </div>
                        <div class="form-group">
                            <label for="observacion">Observación</label>
                            <input type="text" value="<%=t.getObservacion()%>" class="form-control" name="observacion">  
                        </div>
                        <div class="form-group">
                            <label for="claseReserva">Clase reserva</label>
                            <select name="claseReserva"  class="custom-select">
                                <option selected><%=t.getClaseReserva()%></option>
                                <option value="Pago directo">Pago directo</option>
                                <option value="Agencia de pago aplazado">Agencia de pago aplazado</option>
                                <option value="Al Momento">Al Momento</option>
                                <option value="Taxistas">Taxistas</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="numeroTaxi">Numero Taxi</label>
                            <input type="number" value="<%=t.getNum_licencia()%>" class="form-control" name="numeroTaxi">
                        </div>
                        <input type="submit" class="btn btn-info btn-block" value="Editar Viaje Taxista" name="accion">
                    </form>
                </div>
            </div>
        </div>
    </body>
    <jsp:include page="footer.jsp"></jsp:include>
</html>
